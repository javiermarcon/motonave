...
class sfResizedFile extends sfValidatedFile
{
	...
	public function save($file = null, $fileMode = 0666, $create = true, $dirMode = 0777)
	{
		...
		if ($file[0] != '/' && $file[0] != '\\' && !(strlen($file) > 3 && ctype_alpha($file[0]) && $file[1] == ':' && 
($file[2] == '\\' || $file[2] == '/')))
		{
			if (is_null($this->path))
			{
				throw new RuntimeException('Se debe proveer una "ruta" cuando se provee un nombre de archivo relativo.');
			}
		...
		if (!is_readable($directory))
		{
			if ($create && !mkdir($directory, $dirMode, true))
			{
				...
				throw new Exception(sprintf('No se pudo crear el directorio de subida: "%s".', $directory));
			}
		...
		if (!is_dir($directory))
		{
			...
			throw new Exception(sprintf('La ruta de subida "%s" existe pero no es un directorio.', $directory));
		}
		if (!is_writable($directory))
		{
			// the directory isn't writable
			throw new Exception(sprintf('El directorio de subida "%s" no tiene permiso de escritura.', $directory));
		}
