...
class sfMemcacheCache extends sfCache
{
   ...
   *
   * * host:       The default host (default to 127.0.0.1)
  ...
  public function initialize($options = array())
  {
    ...
    else
    {
      ...
      else
      {
        ...
        if (!$this->memcache->$method($this->getOption('host', '127.0.0.1'), $this->getOption('port', 11211), $this->getOption('timeout', 1)))
        {
          throw new sfInitializationException(sprintf('Unable to connect to the memcache server (%s:%s).', $this->getOption('host', '127.0.0.1'), $this->getOption('port', 11211)));
        }
      }
    }
  }
