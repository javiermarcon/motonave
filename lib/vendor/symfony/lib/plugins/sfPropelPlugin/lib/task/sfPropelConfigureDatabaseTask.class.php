...
for a project:
  ...
  [./symfony configure:database mysql:host=127.0.0.1;dbname=example root mYsEcret|INFO]
...
to change the dsn for a specific environment, use the [env|COMMENT] option:
  ...
  [./symfony configure:database --env=dev mysql:host=127.0.0.1;dbname=example_dev root mYsEcret|INFO]

To change the configuration for a specific application, use the [app|COMMENT] option:

  [./symfony configure:database --app=frontend mysql:host=127.0.0.1;dbname=example root mYsEcret|INFO]

You can also specify the connection name and the database class name:

  [./symfony configure:database --name=main --class=ProjectDatabase mysql:host=127.0.0.1;dbname=example root mYsEcret|INFO]
